...
import { microsoftOutlookActief, microsoftPilotActief, microsoftSharePointActief, registreerKoppelfout, voltooiKoppeling } from "@/core/lib/microsoft-connector";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!profiel?.fondsId || !((await microsoftPilotActief(supabase, profiel.fondsId)) || (await microsoftOutlookActief(supabase, profiel.fondsId)) || (await microsoftSharePointActief(supabase, profiel.fondsId)))) return NextResponse.redirect(fallback, { headers: { "Cache-Control": "no-store" } });
