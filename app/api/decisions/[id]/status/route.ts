...
export const POST = withFondsRoute({ hostGuard: "geen", rateLimit: "nog-niet-beoordeeld", audit: { handeling: "decisions.id.status.post" }, capability: "decisions.manage", schema: z.object({ "override_reden": z.unknown().optional(), "reden": z.unknown().optional(), "status": z.unknown().optional() }).passthrough() }, async (ctx, req: NextRequest, params) => {
  try {
