...
export const PATCH = withFondsRoute({ hostGuard: "geen", rateLimit: "nog-niet-beoordeeld", audit: { handeling: "notulen.segmenten.id.patch" }, capability: "notulen.segment.confirm", schema: z.object({ "agendapunt_id": z.unknown().optional(), "bevestigd": z.unknown().optional(), "reden": z.unknown().optional(), "tekst": z.unknown().optional(), "titel": z.unknown().optional() }).passthrough() }, async (ctx, req: NextRequest, params) => {
  try {
...
export const DELETE = withFondsRoute({ hostGuard: "geen", rateLimit: "nog-niet-beoordeeld", audit: { handeling: "notulen.segmenten.id.delete" }, capability: "notulen.segment.confirm", schema: "geen-body" }, async (ctx, _req: NextRequest, params) => {
  try {
    const { id } = params as { id: string };
    const supabase = ctx.supabase;
    if (!(await requireCapability(ctx.gebruikerId, "notulen.segment.confirm"))) {
