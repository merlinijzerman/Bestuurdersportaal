...
export const PATCH = withFondsRoute({ hostGuard: "geen", rateLimit: "nog-niet-beoordeeld", audit: { handeling: "dossiers.id.patch" }, capability: "dossiers.manage", schema: z.object({ "motivering": z.unknown().optional(), "periode_eind": z.unknown().optional(), "periode_jaar": z.unknown().optional(), "periode_start": z.unknown().optional(), "periode_type": z.unknown().optional(), "status": z.unknown().optional() }).passthrough() }, async (ctx, req: NextRequest, params) => {
  try {
    const { id } = params as { id: string };
    const supabase = ctx.supabase;
