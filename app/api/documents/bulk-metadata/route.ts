...
export const POST = withFondsRoute({ hostGuard: "geen", rateLimit: "route-eigen", audit: { handeling: "documents.bulk-metadata.post" }, capability: "documents.metadata.update", schema: z.object({ "document_ids": z.unknown().optional(), "preview": z.unknown().optional(), "wijziging": z.unknown().optional() }).passthrough() }, async (ctx, req: NextRequest) => {
  try {
