...
// ============================================================
export const PATCH = withFondsRoute({ hostGuard: "geen", rateLimit: "nog-niet-beoordeeld", audit: { handeling: "agendapunten.wijzigen" }, capability: "agendapunten.manage", schema: z.object({ "beschrijving": z.unknown().optional(), "categorie": z.unknown().optional(), "motivering": z.unknown().optional(), "tijdsduur_minuten": z.unknown().optional(), "titel": z.unknown().optional(), "verantwoordelijke": z.unknown().optional(), "vergadering_id": z.unknown().optional(), "volgorde": z.unknown().optional() }).passthrough() }, async (ctx, req: NextRequest, params) => {
  try {
...
// ============================================================
export const DELETE = withFondsRoute({ hostGuard: "geen", rateLimit: "nog-niet-beoordeeld", audit: { handeling: "agendapunten.verwijderen" }, capability: "agendapunten.manage", schema: z.object({ "reden": z.unknown().optional() }).passthrough() }, async (ctx, req: NextRequest, params) => {
  try {
    const { id } = params as { id: string };
    const supabase = ctx.supabase;
