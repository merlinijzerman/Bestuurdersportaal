...
export const PATCH = withFondsRoute({ hostGuard: "afdwingen", rateLimit: "nog-niet-beoordeeld", audit: { handeling: "procedures.id.afschriften.afschriftId.patch" }, capability: "procedures.manage", label: "procedures.afschrift.intrekken.PATCH", schema: z.object({ "reden": z.unknown().optional() }).passthrough() }, async (ctx, req: NextRequest, params) => {
  try {
