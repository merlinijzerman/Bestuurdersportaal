...
export const PATCH = withFondsRoute({ hostGuard: "geen", rateLimit: "nog-niet-beoordeeld", audit: { handeling: "procedures.bewijs.wijzigen" }, capability: "procedures.manage", schema: z.object({ "document_id": z.unknown().optional(), "documenttype": z.unknown().optional(), "vereiste": z.unknown().optional() }).passthrough() }, async (ctx, req: NextRequest, params) => {
  try {
...
export const DELETE = withFondsRoute({ hostGuard: "geen", rateLimit: "nog-niet-beoordeeld", audit: { handeling: "procedures.bewijs.verwijderen" }, capability: "procedures.manage", schema: "geen-body" }, async (ctx, _req: NextRequest, params) => {
  try {
    const { id, bewijsId } = params as { id: string; bewijsId: string };
    const supabase = ctx.supabase;
