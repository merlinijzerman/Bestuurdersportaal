...
- **Regime-borging (T4, opgeleverd 2026-08-12).** De retrieval behandelde alle wet als top-autoriteit → de assistent haalde de PW als geldend recht op voor een Wvb-fonds. T4 vult de door T1 gereserveerde plek met een **demoterende** regime-weging (`weegRegime`, `core/lib/weeg-regime.ts`, puur — spiegelt `weegBronsoort`): ná de bronsoort-weging, vóór de representatie-constraints. Chunks met een **tegengesteld specifiek** regime (PW bij een Wvb-fonds, of omgekeerd) zakken naar onderaan; `beide`/`algemeen`/`NULL` (cross-cutting) **nooit**; **geen harde uitsluiting** (PW blijft als aanvullend extern kader). **Data-gedreven, niet in code:** documentfacet `documenten.wettelijk_regime` (gedenorm. naar `document_chunks` via `fn_chunk_denorm*`, teruggegeven door beide zoek-RPC's als return-only kolom), fonds-velden `fondsen.fondstype` + `primair_wettelijk_regime` (compliance/platform-beheerd, niet tenant-writable → op `fondsen`, niet `organisatie_profielen`) en reference-tabel `wettelijk_regime_per_fondstype` (mapping fondstype→regime, compliance-eigenaar). **Prompt-B6** (`bouwRegimeKaderBlok`, dynamisch contextblok `BestuurderContext.regimeKader`) labelt een niet-geldend regime als "[extern kader — niet geldend recht voor dit fonds]", **zonder** verplichte verificatievraag. **Flag `REGIME_WEGING` default AAN** (bewust anders dan T1: gedrag-neutraal zolang de ~104 generieke docs geen regime-tag dragen (`NULL ≡ algemeen`) en `weegRegime` no-opt bij een fonds zonder specifiek regime). Migratie `2026_08_12_t4_regime_borging.sql` (+ROLLBACK); Horizon-seed `primair_wettelijk_regime='pw'`. **Openstaand (governance-gate):** compliance bevestigt de mapping + de exacte Horizon-`fondstype` + juridisch eigenaar generieke bibliotheek; de regime-tagging van de ~104 generieke docs is out of scope (aparte curatie-actie). Zie [`decisions/0162`](./decisions/0162-regime-borging-demotie-niet-geldend-regime.md).
