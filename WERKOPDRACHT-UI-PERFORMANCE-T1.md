...
- Wel (maatregel 7, B7): `@vercel/speed-insights` toevoegen in de root-layout (CSP staat `*.vercel-insights.com` al toe — verifieer dat de bestaande directives volstaan); rapporteer daarnaast de daadwerkelijke regio's van Vercel en Supabase en signaleer een eventuele mismatch (alleen rapporteren, niet migreren).
...
4. Speed Insights levert per-route veldmetingen in het Vercel-dashboard; regio's van Vercel en Supabase zijn gerapporteerd in de terugkoppeling.
