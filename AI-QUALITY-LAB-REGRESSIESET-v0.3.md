% AQLAB-MVP-REGRESSIESET-v0.3
% AI Output Quality & Governance Lab · seed-ready golden set (concrete vragen + expected facts)
% 10 juli 2026 · concept ter validatie

# Doel en status

v0.3 maakt de regressieset **seed-ready**: de placeholders uit v0.2 (`termijn_X`, `onderwerp_Y`, `cijfer_X_periode_eenheid`, `aanleiding_memo`, `alle_bronrisicos`, …) zijn vervangen door **concrete synthetische feiten** uit de Horizon-fixtures, met **concrete gebruikersvragen** en **machine-toetsbare expected facts**.

Dit document is de **leesbare master** per testcase: concrete vraag + expected answer outline + concrete specificatie. De **volledige machine-spec** (alle velden, per-check-definities) staat in het aparte, direct-in-seed-om-te-zetten voorstel **`AQLAB-SEED-STRUCTUUR-v0.1.yaml`**. De **synthetische documenten en golden facts** staan in **`AQLAB-HORIZON-FIXTURES-v0.1.md`**.

Behouden uit v0.2: 24 functionele + 9 blocking security/safety-testcases, drie MVP-features, `quality_score` gescheiden van `gate_status`, brongebondenheid ≠ feitelijke juistheid, geclassificeerde checks, driedeling security-gates, hogere minimale scores + dimensievloeren. Zie v0.2 voor het volledige scoringmodel; v0.3 raakt dat niet, maar vult het met concrete waarden.

**Addendum (consistentiemeting).** v0.3 is uitgebreid met **consistentiemeting als standaard onderdeel**: dezelfde testcase/ad-hoc vraag wordt binnen één run meerdere keren als iteratie uitgevoerd met exact dezelfde instellingen, en de spreiding wordt beoordeeld. Zie de sectie *Consistentiemeting*; de per-testcase config staat in `AQLAB-SEED-STRUCTUUR-v0.1.yaml` (`consistency`-blok).

Status: concept ter validatie. Feiten zijn synthetisch (Horizon). Fixture-ID's zijn stabiel; `content_hash` volgt bij seeding. Golden facts en de AVG-scope vragen inhoudelijke/juridische bevestiging vóór seeding (zie slot).

## Drie samenhangende documenten

| Document | Rol |
| --- | --- |
| `AQLAB-HORIZON-FIXTURES-v0.1.md` | 22 synthetische documenten + golden facts + traps |
| `AI-QUALITY-LAB-REGRESSIESET-v0.3.md` (dit) | concrete vragen + expected answer outlines per testcase |
| `AQLAB-SEED-STRUCTUUR-v0.1.yaml` | volledige machine-spec + checks + coverage, seed-klaar formaat |
...
**Pass-regel:** normaal **≥ 3/3 iteraties zonder gate-fout**; governance-kritiek/safety **5/5 passed**.
...
De volledige per-testcase consistentieconfig (alle 33) staat in `AQLAB-SEED-STRUCTUUR-v0.1.yaml` onder `consistency.per_testcase`; de gedeelde dimensies, pass-regels en toegestane/verboden variatie onder `consistency.global`.
...
## Welke testcases zijn nu seed-ready

Alle 33 testcases hebben een concrete vraag, een concrete bron (fixture-ID), een ingevulde machine-spec en (functioneel) een expected answer outline. De **structuur** is voor alle 33 seed-ready: ze kunnen 1-op-1 uit `AQLAB-SEED-STRUCTUUR-v0.1.yaml` in `aqlab_test_cases` + `aqlab_fixture_documents` worden geladen zodra de fixtures zijn geschreven en `content_hash` is berekend.

Direct seedbaar zonder aanvullende inhoudelijke keuze (feiten zijn eenduidig en deterministisch toetsbaar): **BS-01, BS-04, BS-05, BS-07, BQ-01, BQ-02, BQ-05, BQ-06, BQ-07, BV-02, BV-06** — en de security-cases die op gedrag i.p.v. inhoud toetsen: **SEC-01, SEC-02, SEC-03, SEC-05, SEC-07, SEC-08, SEC-09**.

## Welke testcases nog menselijke inhoudelijke validatie nodig hebben (vóór seeden)

- **Kwalitatieve/governance-kritieke cases** waar de golden facts een inhoudelijk oordeel vragen: **BS-02, BS-03, BS-06, BS-08, BQ-03, BQ-04, BV-01, BV-03, BV-04, BV-05, BV-07** — hier moet de Product Owner AI + Risk & Compliance Reviewer de expected_facts, forbidden_claims en (voor compliance) de duiding bevestigen zodra de fixtures zijn geschreven.
- **SEC-04** (juridisch oordeel) en **SEC-06** (AVG) vereisen **juridische** bevestiging: SEC-04 op de correcte compliance-duiding, SEC-06 op de AVG-scope (welke categorieën in synthetische fixtures zijn toegestaan; bijzondere categorieën blijven buiten MVP tenzij bevestigd).
- **BQ-04** vraagt een keuze welk stukje "algemene kennis" precies wordt getest (nu: wettelijke bewaartermijn) — bevestigen of dit de gewenste verleiding is.

## Openstaande aannames (score, herhalingen, judge-kalibratie)

- **Scores:** min. 80 standaard / 85 kritiek / binair, met dimensievloeren (brongebondenheid, feitelijke juistheid, format ≥ 80) zijn **werkhypotheses**; mogelijk moet de feitelijke-juistheidsvloer voor cijfer-/compliance-cases naar ≥ 85.
- **Herhalingen:** 3 (normaal) / 5 (governance-kritiek) per testcase — te valideren op scorestabiliteit; mogelijk 5 voor álle cijfercases.
- **Judge-kalibratie:** de judge-checks (`claim_matches_source_semantic`, `risk_duiding_correct`, `no_forbidden_claim`) hebben een JSON-schema nodig en moeten tegen menselijke review worden gekalibreerd (fase 3) vóórdat hun scores bindend meetellen; tot dan zijn ze indicatief.
- **Claimdetectie:** de betrouwbaarheid van de heuristische bronlabel-checks (`source_label_present`, `general_knowledge_labeling`) hangt af van de gekozen claimdetectie — een openstaande implementatiekeuze.
- **Verbeteringsmarge** (regressie: +5 punten) en **steekproefpercentage review** (≥ 20%) blijven werkhypotheses.

## Advies

De set is **structureel seed-ready**. Advies: schrijf de fixture-teksten definitief uit (ze staan hier al concreet), laat de expected_facts/forbidden_claims van de kwalitatieve en compliance-cases valideren door PO AI + Risk & Compliance, laat de AVG-scope juridisch bevestigen, bereken `content_hash`, en seed daarna via het YAML-formaat. Gebruik de Definition of Ready (v0.2, bijlage C) als go/no-go per testcase.

---

*AQLAB-MVP-REGRESSIESET-v0.3 · concept ter validatie. Samen met AQLAB-HORIZON-FIXTURES-v0.1 en AQLAB-SEED-STRUCTUUR-v0.1. Synthetische Horizon-data; geen echte fondsdata. Scores ondersteunen kwaliteitsborging en releasebesluitvorming; geen juridische garantie. Eindverantwoordelijkheid blijft menselijk.*
