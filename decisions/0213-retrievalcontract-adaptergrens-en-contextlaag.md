...
2. **Wat buiten de grens valt, wordt bevroren én geclassificeerd — en de scan is transitief.** Het register (`tests/cross-tenant/retrieval-contextbronnen.expected.json`, gate `F4-context`) loopt de **hele importgraaf** vanaf de chatroute af (106 bestanden, 13 lezers) en pint de vier klassengroottes 5/18/7/3. Een bereikte tabel **zonder klasse maakt de gate rood**: classificeren is een ontwerpoordeel en mag niet meeliften. De eerdere directe-import-scan miste drie lezers, waaronder `core/lib/parent-context.ts`, dat `document_chunks` rechtstreeks leest. Dit is de tegenhanger van de retrievalcensus (17 bestanden), die in T2 juist krimpt tot adapter en orkestratie.
...
6. **Een permissionproof die niet toetsbaar is, is geen proof.** `capabilities().permissionProof` was een boolean waarmee de adapter iets over zichzelf beweerde; `Bronresultaat` droeg geen feitelijk rechtenbewijs, dus de orkestratie kon niet afdwingen wat het ontwerp belooft. Elk resultaat draagt daarom `toegangscontrole { toegestaan, gecontroleerdOp, basis, bronconfiguratieVersie }`, en een toelatingspoort vóór de selectie weigert wat daar niet aan voldoet. **Versiecontrole en autorisatiecontrole zijn gescheiden bewijzen met gescheiden tijdstippen**: `versie.gecontroleerdOp` wordt niet langer ook als permission-checktijd gebruikt — anders is een verlopen rechtencheck op een nog geldige versie niet te herkennen.
...
10. **Het hybride pad krijgt een eigen tranche T1b** (R3): embeddingstub plus geëmbedde fixtures en een hybride golden, vóór enige productiecode in T2-1. Zonder die stub karakteriseren de goldens alleen de FTS-terugval, terwijl het hybride pad in productie het primaire pad is.
...
**Datamodel/migraties (herzien).** T1 raakt niets. Voor T2 is nu één migratie **besloten** (R4): `correlationId` in de SQL-allowlist van `meta_basisniveau()`, idempotent en mét check, plus een regel in `supabase/checks/allowlist-grants.tsv` als er een object of grant wijzigt.
