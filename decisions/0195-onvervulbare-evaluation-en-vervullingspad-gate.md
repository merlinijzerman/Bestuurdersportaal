...
**6. Eenmalige, gemeten I7-uitzondering voor de legacycorrectie.** De historische
seed blijft byte-ongewijzigd: een wijziging van een reeds toegepaste migratie zou
geen Preview-data corrigeren en veroorzaakt migratiedrift. De correctie gebeurt
daarom uitsluitend via
`2026_08_29_zz_0195_verwijder_onvervulbare_templatevereisten.sql`.

Op 29-08-2026 is vóór P1b de projectie op `template_code` gemeten, omdat
`procedures.template_versie` nog niet bestond en P1b de bestaande dossiers naar
`1.0.0` zou backfillen:

- Preview: **0** dossiers.
- Productie: **3** dossiers; de opdrachtgever heeft bevestigd dat deze niet in
  gebruik zijn.

De eigenaar staat daarom één inhoudelijke wijziging van de gepubliceerde
`beleidswijziging_beleggingsbeleid@1.0.0` toe: precies de `evaluation`-rij op
stap 6 verwijderen. De voorwaartse migratie en haar rollback accepteren alleen
de gemeten aantallen 0 (Preview) of 3 (productie); elk ander aantal faalt. De
I7-trigger `trg_req_versievast` wordt uitsluitend transactioneel tijdelijk
uitgezet, nooit via `session_replication_role`, en is vóór commit aantoonbaar
weer actief. Lokaal is vastgesteld: 1 dossier faalt; 0 en 3 doorlopen zowel
forward als rollback→forward met een actieve I7-trigger. Deze uitzondering
...
- **Geen actief dossier geraakt:** Preview heeft geen dossier uit deze template;
  productie bevat drie niet-gebruikte dossiers, expliciet onderbouwd in besluit
  6. De uitzondering is daardoor meetbaar en begrensd, niet stil.
...
- Correctie en bewijs: `supabase/migrations/2026_08_29_zz_0195_verwijder_onvervulbare_templatevereisten.sql`, de bijbehorende rollback en check, `MIGRATIEDRAAIBOEK-EPIC-P-PREVIEW.md`, commit `c425505`.
