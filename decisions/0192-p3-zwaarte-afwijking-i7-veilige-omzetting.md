...
## Regels bij PR-C (afronden met afwijking) — regels, geen uitzonderingen

PR-C bouwt het afronden-met-afwijking (§5.1). Twee dingen wijken af van een letterlijke lezing van het ontwerp; ze worden hier als **regel** vastgelegd, zodat de volgende bouwer ze niet opnieuw hoeft af te wegen en wat eraan wordt toegevoegd aan dezelfde regel wordt getoetst.

**Atomariteitsregel.** Wat een bewering doet over verantwoording hoort in de transactie; afgeleide toestand die je opnieuw kunt berekenen hoort erbuiten. §5.1 noemt vijf handelingen in één transactie; uitgevoerd zijn de **eerste vier atomair** (statuswijziging + de vier afwijkingskolommen, de snapshot, de `procedure_log`-regel, het governance-event — samen de bewering "deze stap is afgerond terwijl dit ontbrak, met deze motivering, door deze persoon") en de **vijfde als herstelbare cascade** (`herberekenActiveerbaarheid` + downstream-activering), omdat activering afgeleide toestand is en geen verantwoordingsfeit. De keuze valt op de regel, niet op het precedent van de bestaande (niet-transactionele) afronding. Volledige atomariteit in SQL viel af op zwaardere grond dan omvang: het zou de grafieklogica naast `core/lib/procedure-activatie.ts` porten — twee implementaties van dezelfde regel die uiteen kunnen lopen. Drie voorwaarden, anders is de afwijking niet verdedigbaar:
1. Een falende cascade wordt niet ingeslikt: `pasActivatieCascadeToe` schrijft bij falen een luide `procedure_log`-regel (`activatie_achterstand`) met verwijzing naar de stap; de route waarschuwt de aanroeper.
2. De herberekening is idempotent en zonder speciale rechten opnieuw aanroepbaar; een periodieke controle in de vorm van `supabase/checks/…driftmeting` hoort de achterstand op te sporen (nog te schrijven vóór go-live).
3. De tussentoestand is aantoonbaar onschadelijk: een nog-niet-geactiveerde stap leidt nooit tot een verkeerde vervuldheids- of statusconclusie — expliciet getoetst in de faalpad-scenario's van de gedragstoets (#6).

**Contractwaarde-regel.** Een contractwaarde wordt opgenomen tegen een draaiende server of hij wordt niet opgenomen; een voorspelde waarde komt nooit in het verwachtingsbestand, ook niet als de uitkomst deterministisch is. De afwijking-route declareert de capability route-dragend (W7-teller 113 → 114), maar het negatieve 403-contract (beheerder/bureau) wordt pas bij de stack-run opgenomen — niet met de hand voorspeld. Drie voorwaarden, anders wordt het gat onzichtbaar in plaats van uitgesteld:
1. De openstaande opname is machineleesbaar: `tests/karakterisering/uitgestelde-opnames.json` + `uitgestelde-opnames.test.ts` (een uitgestelde route mag nog geen scenario/opname hebben).
2. Die lijst moet leeg zijn vóór P6 — release-blokkade [#211](https://github.com/merlinijzerman/Bestuurdersportaal/issues/211), naast [#192](https://github.com/merlinijzerman/Bestuurdersportaal/issues/192), [#207](https://github.com/merlinijzerman/Bestuurdersportaal/issues/207), [#208](https://github.com/merlinijzerman/Bestuurdersportaal/issues/208); een route waarvan het negatieve contract nooit is waargenomen mag niet promoveren.
3. De bedoelde divergentie is benoemd bij de W7-teller (114 declaraties, opgenomen 403-cellen op de oude set) zodat de volgende lezer het niet als bevinding aanmerkt. De gate wordt intussen wél getoetst: `tests/cross-tenant/afwijking-rolgate.test.ts` bewijst beide richtingen zonder stack.

## PR-C review — en de derde van dezelfde familie

De reviewset (vier sporen) tegen de container-DB leverde meer op dan losse fixes; twee dingen zijn leerzaam genoeg om vast te leggen.

**De snapshot-pin was een vals-negatief — de klasse die hij juist moet vangen.** De eerste versie deelde zeven smalle, disjuncte vectoren tussen de SQL- en de TS-helft en oefende maar één van de acht gebonden-feit-bronnen uit; geen enkele vector liet een template- en een instantie-vereiste dezelfde sleutel delen, en er zat geen actieve conditional in. De pin bevestigde alleen de rekenkunde, niet de bronset, de uitsluiting, de activatie of de ambiguïteit — en juist daar zaten **twee echte divergenties** met `core/lib/decision.ts`: (1) de `v_dubbel`-ambiguïteitstelling paste het uitsluitings- en activatiefilter niet toe, waardoor een uitgesloten of inactieve template-tweeling een botsende actieve instantie stil fail-closed maakte; (2) de field-tak `decision.besluitvraag` telde de lege string als vervuld (fail-open) waar `decision.ts` `!!besluitvraag` gebruikt. Beide zijn gecorrigeerd; de pin is gehard tot hij ze aantoonbaar vangt — de SQL-helft oefent nu alle acht bronnen (`min_aantal=8`, dus een ontbrekende bron bijt), de ambiguïteit-met-uitsluiting en `besluitvraag=''` uit, mutatie-getest. Les: een pin die alleen het makkelijke geval dekt, is dezelfde vals-negatief-klasse als de schema-only stub (dit besluit, §Context) — hij moet aantoonbaar bijten, niet alleen groen zijn.

**De `fn_stap_open_per_zwaarte`-vondst is de derde van één familie.** De read-only snapshothelper was `SECURITY DEFINER` (bypast RLS), `grant execute` aan `authenticated`, en droeg **geen fondscheck**. Een voorzitter van fonds A kon haar met een vreemde `stap_id` direct aanroepen en de open vereisten (labels + sleutels) van een stap in fonds B uitlezen — een cross-tenant leesleak. Dit is dezelfde klasse als een eerdere reviewbevinding (bevinding 2), alleen op een helperfunctie waar niemand aan dacht. **Twee keer dezelfde fout in één tranche is geen oplettendheidskwestie maar een structureel gat.** De directe fix is klein (grant ingetrokken; niets buiten de afrondfunctie roept de helper aan, en die draait als eigenaar). De structurele remedie is een eigen gate — [#212](https://github.com/merlinijzerman/Bestuurdersportaal/issues/212), in de vorm van [#209](https://github.com/merlinijzerman/Bestuurdersportaal/issues/209): elke `SECURITY DEFINER`-functie met een grant aan `authenticated` moet aantoonbaar zelf een `auth.uid()`-afgeleide fonds- én rolcontrole doen, of expliciet op een allowlist met motivering staan. Statisch op de definitie te toetsen, plus een gedragstoets. Niet meeliftend op P3.

De overige bevindingen zijn verwerkt zonder eigen besluit: `for update` op de stap tegen dubbel-afronden, eigen SQLSTATE `PC002` zodat een kale `23514` (een echte constraint) geen schema-tekst lekt, de cascade idempotent (geen dubbele afrondnotificatie bij herstel), en de UI-bevestiging die niet meer vastloopt als client en server over "kritiek" verschillen.

## Referenties

- Migraties: `supabase/migrations/2026_08_27_p3b_01_zwaarte.sql`, `…_p3b_02_booleans_generated.sql` (+ rollback); `…_p3c_01_afwijking_kolommen.sql`, `…_p3c_02_fn_afronden_afwijking.sql` (+ rollbacks). Fixture: `scripts/testdb-i7-fixture.sql`. Helpers: `core/lib/requirement-zwaarte.ts`, `core/lib/procedure-activatie-cascade.ts`. Functies: `fn_stap_open_per_zwaarte`, `fn_stap_afronden_met_afwijking`.
