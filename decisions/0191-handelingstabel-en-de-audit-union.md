...
- De **15 bestuurlijk-gap-handlers** (stemmen/agenderen/bewijs/vergaderingen/notulen/inbreng/organisatieprofiel/documents-status) hebben vandaag **geen `governance_events`-event**. #183 voegt de route-eigen write toe; **`audit: "governance-events"` mag pas daarná** (de assertie is fail-closed vanaf dag één).
- De census in de W11-scope: **22 bewijsketen · 58 enig spoor · 36 geen** (was 22 · 34 · 38); `instellingen` en `stuurinformatie/beheer` vielen uit de "zonder spoor"-lijst (trigger-spoor).
