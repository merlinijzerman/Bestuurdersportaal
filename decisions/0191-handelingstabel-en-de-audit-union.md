...
>
> **Gemeten stand van de machine-kant (`origin/preview`, 2026-08-26): nul van de zeven machine-SPECs schrijft vandaag `platform_event_log`.** De vier orchestrator-workers roepen `logAttempt`/`logResultGegarandeerd` niet aan; `monitoring/snapshot` *leest* `platform_event_log` (het is de gat-detector, `.select().eq("fase","attempt")`), het schrijft het niet. De enige schrijver is `platform-audit.ts`, uitsluitend bereikt via `platform-wrapper.ts` → `aqlab/audit/[exportId]` — géén machineroute. Gevolg: in een byte-identieke #183a kan `"platform-event-log"` er nog niet staan (dat zou de fail-closed assertie rood zetten), net als `"governance-events"` op een bestuurlijk-gap-route dat zou. Daarom een **tweede drager, symmetrisch aan `ketengebeurtenis_vereist`:** `spoor_vereist` = de 5 worker-SPECs / 9 declaraties waarvan de `platform_event_log`-write openstaat, machineleesbaar in `audit-inventaris.json`, per SPEC gemeten (GET+POST delen één `SPEC`/`draai`). Rood tot #183b-machine de write landt; dán meet de drager zichzelf leeg.
...
- **CORRECTIE op een eerdere telling (was 15).** Vier handlers die als "bestuurlijk-gap" stonden hébben wél een spoor — `procedure_log`, geschreven door `trg_procedure_bewijs_audit` (fail-closed) bij elke mutatie op `procedure_bewijs`. De meting miste die trigger; `BASE_TRIGGER` kent `procedure_bewijs` nu, en de anti-drift-test bewaakt hem. Het waren geen 3 maar 4: naast de drie `procedures/[id]/bewijs`-handlers INSERT'et óók `stemmingen/[id]/sluiten` zelf een `procedure_bewijs`-regel.
  ...
  - **TOETSREGEL (AMENDEMENT 2026-08-27, #183b-analyse) — wanneer volstaat een domeinspoor?** Een bestaand domeinspoor volstaat als `governance_events`-vervanging **alleen als dat spoor hetzelfde *soort* feit vastlegt als de handeling.** `procedure_log` bij een procedurehandeling: ja — zelfde soort feit. `document_metadata_log` bij een bestuurlijke notulen­bevestiging: **nee** — het spoor bestaat en is fail-closed, maar het is document-/RAG-gescopet en beantwoordt een andere vraag dan de besluit-provenance-keten. Het feit dát een bestuurder verklaart "dít is wat er is gezegd/besloten" is geen bestandsmetadata. Toepassing: de notulen-handlers (#5 bevestig, #6 verwijder) **blijven** in de twaalf en krijgen een `governance_events`-event bovenop (in hun bestaande atomische RPC), net als `sluiten` bovenop `procedure_log`. Met deze regel is de vólgende zo'n classificatie een **toepassing, geen besluit**.
...
- Census in de W11-scope, gecorrigeerd: **22 bewijsketen · 62 enig spoor · 32 geen** (klasse: bewijsketen 22 · domein 40 · bestuurlijk-gap 11 · operationeel 8 · geen 7 · machine 6). Eerder 22 · 58 · 36; daarvoor 22 · 34 · 38.
  - **Let op de eenheid.** Deze census telt **state-changing handlers** (POST/PATCH/PUT/DELETE) — 94 stuks, `machine 6` = de 6 machine-POST's. De #183a-codemod vult daarentegen het `audit`-veld op **álle 124 declaraties** (incl. de GET's die geen state veranderen). Die twee tellingen niet verwarren: de #183a-bevriezing is **81 `AuditSpec` · 43 `"geen"` · 0 `"platform-event-log"`** (81 = alle te-auditen handlers = bewijsketen 22 + domein 40 + bestuurlijk-gap 11 + operationeel 8; 43 = 7 sc-zonder-spoor + 24 fonds-GET + **12 machine**). De **eindstand** ná #183b-machine is 81 · 34 · 9 — dán mét gemeten writes. Bevriezing ≠ eindstand; dat onderscheid ís de a/b-splitsing.
- **Machine-`audit` (per SPEC, niet per methode).** 7 SPEC-objecten, 12 declaraties; GET+POST delen in vijf bestanden letterlijk één `const SPEC` + één `draai`, dus de `audit`-waarde is per SPEC — per-methode splitsen zou de SPEC splitsen (structuurwijziging, geen byte-identieke codemod). #183a bevriest **alle 12 op `"geen"`** (eerlijk: vandaag schrijft geen enkele machine-SPEC `platform_event_log`). #183b-machine voegt de `logResultGegarandeerd`-write toe aan de **5 worker-SPECs** (aqlab/worker · afschrift-worker · ingest-worker · semantische-extractie · monitoring/snapshot = 9 declaraties) en flipt ze naar `"platform-event-log"`; de **2 probes** (`platform/healthz`, `healthz/ping` = 3 declaraties) blijven `"geen"`. De drager `spoor_vereist` (§6) staat rood tot die writes landen.
  - **Twee dingen om vóór #183b-machine uit te zoeken (voorwaarde in dat ticket, geen verrassing achteraf):** (1) **Serialisatie.** `platform_event_log` draagt een hashketen met één kop (`trg_platform_event_hash`, `fn_platform_event_chain_assert_valid`, `2026_08_15_platform_event_chain_head.sql`). Vier crons die parallel/overlappend per run appenden kunnen op die kop botsen — lock-contentie of ketenbreuk. Meet vóór het toevoegen hoe de kop wordt gelezen/geschreven en wat er bij twee gelijktijdige appends gebeurt. (2) **Volume/retentie.** `platform_event_log` is append-only + hash-geketend en heeft — als `governance_log_inhoud` (**R-24**, staat open) — voor zover bekend geen retentiebaan. Workers die per run schrijven maken er een operationele stroom van i.p.v. een laagvolume-auditspoor; dat raakt R-24.
...
- **VLAGKOPPELING (voorwaarde 5c) — AMENDEMENT 2026-08-27 (#183b-analyse):** `ENFORCE_AUDIT=on` mag pas als **drie** dingen waar zijn, niet twee: (1) `ketengebeurtenis_vereist` = 0, (2) `spoor_vereist` = 0, én (3) **de retentiebaan voor `handelingen_log` draait aantoonbaar.** Reden: `fn_handelingen_snoei()` (de 90-dagen-snoei uit §4) bestáát met `EXECUTE` voor `service_role`, maar **niets roept hem aan** — geen `pg_cron`, geen Vercel-cron; de enige verwijzingen zijn de grant-allowlist en de rollback. Zolang `ENFORCE_AUDIT` uit staat is `handelingen_log` leeg en is er geen blootstelling. Zodra de vlag aangaat, vult de tabel zich met gedragsdata onder een **gedocumenteerde termijn van 90 dagen die niemand uitvoert** — en een vastgelegde bewaartermijn die niet wordt nageleefd is onder de AVG slechter dan geen termijn (beleid opgeschreven, niet uitgevoerd). De baan + zijn resultaatbewaking zijn uitgewerkt in [`TICKET-RETENTIESNOEI-DRIE-APPEND-ONLY-TABELLEN.md`](../TICKET-RETENTIESNOEI-DRIE-APPEND-ONLY-TABELLEN.md); die toetst per tabel of er rijen ouder dan termijn+marge bestaan (uitkomst, niet "draaide de job"). Voorwaarde 3 hoort in dezelfde gateset als voorwaarde 1/2.
- **VLAGKOPPELING (voorwaarde 5b):** na #183a dragen de 12 `ketengebeurtenis_vereist`-handlers een `AuditSpec` en schrijven ze een `handelingen_log`-regel — ze *lezen* dan als geaudit terwijl hun `governance_events`-ketengebeurtenis nog ontbreekt (het model oogt compleet met de bevinding eronder). De gate rood maken in #183a kan niet (dan mergt #183a niet), dus de koppeling ligt aan de **vlag**: **`ENFORCE_AUDIT=on` is pas toegestaan als *beide* dragers leeg zijn — de bewijsketen-gap-gate (`ketengebeurtenis_vereist`, alle 12 hebben hun `governance_events`-write) én de machine-drager (`spoor_vereist`, alle 5 worker-SPECs schrijven `platform_event_log`).** Eén vlag, twee halve waarheden eronder: zonder de tweede gate zou `ENFORCE_AUDIT=on` mógen terwijl vijf service-role-crons met eigenaarsrechten nog geen spoor laten. `security/OMGEVINGEN-RUNBOOK.md` bestaat wél, maar de vlagkoppeling staat er nog niet in — leg hem daar vast. Let op: de [#197](https://github.com/merlinijzerman/Bestuurdersportaal/pull/197)-aanname "vlagstand meetbaar via healthz" werkt op Preview **niet** door de Vercel Deployment Protection heen: `curl` krijgt de Vercel-challenge-HTML in plaats van API-JSON. De check vereist dus de Vercel Protection-Bypass-header (of de Vercel-env-config als autoritatieve bron), anders is "meetbaar via healthz" een dode verwijzing.
...
> **⚠ DELTA-3 — vrije tekst in de gecureerde `governance_events`-payloads (#183b spoor T).** De 12 ketengebeurtenissen dragen bewust géén rijdumps (dataminimalisatie), maar enkele gecureerde velden zijn **wel vrije tekst en permanent/onredigeerbaar**: `agendapunt.titel`, `stemming.vraag`, `stemming.ingetrokken_reden`, en de document-deactivatie-`reden`. Dit zijn bewuste governance-feiten (het "wat" van een bestuurlijke handeling, per 0192), geen ongelukje van `to_jsonb`. Maar ze kunnen persoonsgegevens bevatten en staan permanent in de keten — hoort **in dezelfde DPIA-delta** bevestigd, niet stilzwijgend meegenomen. `stem_uitbrengingen` legt bewust géén `keuze`/motivering vast (optie 2).

> **⚠ DELTA-2 — `actor_naam` in de permanente bewijsketen (#183b spoor T, besluit 0192).** De 12 nieuwe `governance_events`-ketengebeurtenissen dragen `actor_naam` als **momentopname** (net als de 20 bestaande writes, `ctx.naam ?? null`) — bewust géén live join naar `profielen`, want een keten die de naam moet ophalen uit een muteerbare tabel zegt "iemand" zodra dat profiel verandert of verdwijnt. Gevolg: een **naam** (persoonsgegeven) staat **permanent, append-only en onveranderlijk** in de keten. Dat is een bewuste opname, hoort in **dezelfde DPIA-delta** als de §4-verruiming hierboven, en is een andere kwestie dan R-22 (dat de hash de actor niet dekt — dat gat blijft, en het weglaten van `actor_naam` zou het niet repareren, alleen het record dunner maken).
