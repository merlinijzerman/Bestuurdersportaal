...
- **Bewust geaccepteerde schuld:** `platform_event_log.identity_id` is nullable voor sessieloze security-events (een aparte security/auth-tabel is P9-scope); de TOTP-enroll toont de door Supabase meegeleverde QR-code (de `qr_code`-SVG uit de enroll-response — géén QR-library toegevoegd) met de sleutel als tekst-fallback; 11b (concurrency) is niet pure-SQL automatiseerbaar en blijft een handmatige twee-sessie-verificatie. **Auto-profiel vs. 3b (waargenomen 23-06-2026):** de live DB maakt bij elk nieuw auth-account automatisch een `profielen`-rij (mechanisme staat niet in de migraties/code — vermoedelijk een losse trigger op `auth.users`), waardoor een vers platform-account meteen als tenant wordt geweigerd. Bij de eerste-beheerder-bootstrap is dat auto-profiel handmatig verwijderd ([`scripts/platform_bootstrap_identiteit.sql`](../scripts/platform_bootstrap_identiteit.sql), stap 0b). Root cause bevestigd: trigger `bij_registratie` → `maak_profiel()` (SECURITY DEFINER). Fix klaargezet in [`2026_06_23b_maak_profiel_platform_skip.sql`](../supabase/migrations/2026_06_23b_maak_profiel_platform_skip.sql) (guard op user-metadata `{"platform": true}` + trigger alsnog getrackt; tenant-onboarding ongewijzigd), nog te draaien op live.
