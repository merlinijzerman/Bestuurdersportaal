...
11. **Procedures iteratie 2** — extra templates (Uitbestedingsreview, Incident-meldplicht DNB), in-app template-editor zodat beheerders zonder code-deploy templates kunnen toevoegen/wijzigen, en koppeling van procedures aan vergaderingen + agendapunten (kolommen `vergadering_id` + `agendapunt_id` op `procedure_besluiten` bestaan al, alleen UI nog niet). Plus eigenaars-FK naar `auth.users` i.p.v. de huidige tekst-velden. Plus AI-suggesties bij besluit-formulering en bewijs-completeness checks.
...
- **29 april 2026 (laatst++)** — **Procedures iteratie 1 — werkende implementatie**. Zeven nieuwe Supabase-tabellen (`procedures`, `procedure_eigenaars`, `procedure_stappen`, `procedure_checklist`, `procedure_bewijs`, `procedure_besluiten`, `procedure_log`) met RLS-policies fonds-breed. Drie pagina's onder `/procedures`: lijst (lopend + afgerond met voortgangsbalken), detail met step-rail + actieve-stap-paneel (checklist toggle, bewijs toevoegen, besluit vastleggen, stap voltooien) + audit-log, en nieuwe-procedure-form met template-picker en co-eigenaars-multi-select. Vijf API-routes voor create / step-status / checklist-toggle / bewijs / besluit, alle met log-events. Beleidswijziging-template hardcoded in `lib/proces-templates.ts` (6 stappen, 18 checklist-items, snapshot-pattern bij start). Validatie bij stap-voltooien: alle checklist voldaan + bewijs aanwezig (bij vereist) + besluit vastgelegd (bij vereist). Migratie + demo-seed: `supabase/migrations/2026_04_29_procedures.sql`. Sidebar-link toegevoegd onder Bestuur. TypeScript-check exit 0.
...
*Laatst bijgewerkt: 29 april 2026 (release Procedures iteratie 1 + eerdere Risicomatrix iteratie 1 + Horizon/oordeelsvorming).*
