...
*Laatst bijgewerkt: 19 mei 2026 — Vergaderingen tranche 1 (Vergader-basics) opgeleverd. Migratie `2026_05_18_vergadering_basics.sql` gedraaid op live Supabase; deploy naar Vercel uitgevoerd. Drie verbeterthema's actief: klikbare documenten op agendapunten, vrije notitieruimte in voorbereiding, en agendapunt-CRUD (wijzigen, verplaatsen, verwijderen, herstellen) met motivering-trigger ≥10 tekens bij ≥1 bijdrager. Drie nieuwe notificatietypes (`agendapunt_gewijzigd/_verplaatst/_verwijderd`) en helper `notifyAgendapuntBijdragers` in `lib/notifications.ts`. `tsc --noEmit --skipLibCheck` groen. **Volgende stappen** liggen klaar: (a) tranche 2 (Stemmingen) zoals beschreven in `VERGADERINGEN-V2-ONTWERP.md` §7 — datamodel + RLS, starten/uitbrengen/sluiten/intrekken, volmacht met `uitgebracht_door`/`stemgerechtigde_id`-splitsing, dissent-prompt voor default-alternatieven, koppeling Decision Object via `procedure_bewijs.stemming_id`, auditdossier-uitbreiding; (b) procedure-audit-followups conform `PROCEDURE-MVP1-AUDIT.md` v1.1 — restrictive-RLS op `decision_ai_interactions`, FK `governance_events.decision_id` naar `on delete restrict`, `chk_bronnen_array` + JSON-schema-validatie, `?trigger=…`-parameter op auditdossier-route. **Parallelle workstream security/compliance**: audit uitgevoerd, hardening-roadmap (routes A/B/C) vastgelegd, plan `SECURITY-ROUTE-A-PLAN.md` voor pilot-klaar baseline klaar voor uitvoering; WP1+WP6 zijn gedaan (Dag 1), WP2-WP5 + WP7 + WP8 staan klaar.*
