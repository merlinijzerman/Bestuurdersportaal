...
*Laatst bijgewerkt: 8 mei 2026 — Decision Object MVP-1 in zijn geheel klaar (subfases 1A t/m 1E plus scope-uitbreidingen vereisten-strook en bewijs-upload). Drie 1D-migraties + één readiness-fix-migratie zijn op live Supabase gedraaid. 1E auditdossier-export operationeel via `GET /api/decisions/[id]/auditdossier?versie=…&formaat=…` met HTML/JSON en live/snapshot. UX-rebalancing: dossier-blokken in uitklapbaren onder de body-grid, `DossierStatusStrip` als compacte overall-status-bar. Drie hotfixes meegenomen: procedures.status-sync, ActieveStapPaneel state-sync via useEffect, en `dynamic = 'force-dynamic'` op de procedure-detailpagina. `tsc --noEmit --skipLibCheck` groen op alle wijzigingen. Volgende stap: deploy naar Vercel + demo-toetsing met bestuurssecretaris (zeven vragen in §7.3 ontwerpdoc) als input voor MVP-2-scope. **Parallelle workstream security/compliance**: audit uitgevoerd, hardening-roadmap (routes A/B/C) vastgelegd, concreet plan `SECURITY-ROUTE-A-PLAN.md` voor pilot-klaar baseline (~2 werkweken, 8 werkpakketten) klaar voor uitvoering; eerste stap WP1+WP6+WP7 op dag 1.*
