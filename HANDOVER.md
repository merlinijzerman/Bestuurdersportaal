...
- **Fase 1D — aannames + risico's + dissent + voorwaarden + acties + besluitregistratie + status-overgangen via readiness-gate + vereisten-strook + bewijs-upload**: code klaar op 8 mei 2026, `tsc --noEmit` groen. **Twee migraties nog draaien in Supabase Dashboard → SQL Editor**: [`2026_05_08_phase_1d_besluit_uitbreiding.sql`](./supabase/migrations/2026_05_08_phase_1d_besluit_uitbreiding.sql) (voegt `procedure_besluiten.decision_id` + `verworpen_alternatieven` toe) en [`2026_05_08_phase_1d_bewijs_documenttype.sql`](./supabase/migrations/2026_05_08_phase_1d_bewijs_documenttype.sql) (voegt `procedure_bewijs.documenttype` toe). Vóór code-deploy moeten beide gedraaid zijn — anders crasht het besluit-formulier en de bewijs-upload op onbekende kolommen. Detail van wat er aan code/UI staat: zie release-historie hieronder.
- **Fase 1E — auditdossier-export (HTML/JSON)**: nog te doen. Server-route die de live `DecisionDossierView` of een bestaande `decision_audit_snapshots`-rij rendert in HTML (printbaar) of JSON (machine-consumption) met `?versie=actueel` of `?versie=besluitmoment`. Geen schema-wijzigingen nodig — alle benodigde tabellen en de view-builder bestaan al sinds 1A. Wel een nieuwe API + UI-knop op het Decision Object om een dossier te exporteren.
...
- *"Lees HANDOVER.md en PROCEDURE-MVP1-ONTWERP.md. Ga verder met Fase 1E: auditdossier-export — server-route die de live `DecisionDossierView` rendert in HTML (printbaar) of JSON (machine-consumption), met `?versie=actueel` of `?versie=besluitmoment` (uit `decision_audit_snapshots`). Plus een export-knop in het Decision Object."*
- *"Lees HANDOVER.md. Voer de demo-toetsing uit op de huidige UI (zeven vragen in §7.3 van het ontwerpdoc) en leg per vraag vast wat de bestuurssecretaris zei — bruikbaar voor latere iteratie of MVP-2-scope."*
...
*Laatst bijgewerkt: 8 mei 2026 — Decision Object MVP-1D klaar (vier sub-pakketten 1D-1 t/m 1D-4: aannames + risico's + dissent + voorwaarden + acties + besluitregistratie + status-overgangen via readiness-gate + vereisten-strook + bewijs-upload). Tien nieuwe API-routes, zes nieuwe componenten + uitbreiding op bestaande, twee nieuwe migraties (`2026_05_08_phase_1d_besluit_uitbreiding.sql` en `2026_05_08_phase_1d_bewijs_documenttype.sql`) — beide nog te draaien in Supabase Dashboard vóór code-deploy. `tsc --noEmit --skipLibCheck` groen. Volgende stap: migraties draaien + Fase 1E (auditdossier-export).*
