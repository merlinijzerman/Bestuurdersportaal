...
> **Status**: **AS-BUILT** t/m AQL-6 (AQL-1..4 assurance/release/audit gereleased 2026-07-10; AQL-5 console-UX & variantbeheer 2026-07-11, besluit 0062; **AQL-6 multi-provider generatie + reasoning-modellen 2026-07-12, besluit 0064** — zie §2.5 as-built-noten). Dit doc is design-laag en kan op detail achterlopen (`CLAUDE.md`).
> **Datum**: 2026-07-10
...
> **[AS-BUILT — AQL-6, besluit [0064]] — multi-provider + reasoning-modellen.** De allowlist heeft nu een **provider-dimensie** (`ModelProvider` = anthropic/openai/mistral) en `model_provider` draagt de **echte** provider (niet meer hardcoded `anthropic`). Challengers: OpenAI-chat `gpt-4.1`/`-mini`/`gpt-4o`/`-mini`, OpenAI-**reasoning** `gpt-5`/`-mini`/`-nano` (`redeneermodel`), Mistral `mistral-large-latest`. **Baseline blijft Claude, judge blijft `claude-opus-4-8`.** De generatie loopt via een provider-abstractie (`lib/llm-providers/*`, adapters anthropic/openai/mistral); **retrieval/RAG + [Bron N]-labeling swappen niet mee**. De **config-hash blijft provider-onafhankelijk** (modelnamen zijn provider-uniek → geen re-seed); provider wordt bevroren op de config **én** per output (`aqlab_run_outputs.model_provider`, migratie `2026_07_12_aqlab_7_run_outputs_provider.sql`). **Reasoning-modellen** hebben een eigen API-contract (`max_completion_tokens` i.p.v. `max_tokens`, temperature/top_p vergrendeld, `reasoning_effort`-knop minimal/low/medium/high) als extra reproduceerbare, bevroren variant-as: velden **`reasoning_effort_requested`** (config) + **`reasoning_effort_effective`** (per output), migratie `2026_07_12_aqlab_8_reasoning_effort.sql`; telt in de config-hash alleen mee als het gezet is. **Data-scope (hard):** externe providers uitsluitend op de synthetische golden set — geen echte fondsdata tot de EU-dataresidentie-migratie; endpoints uit base-URL-constanten (EU-swap = config-wissel). `aqlab_*` blijft provider-globaal/deny-by-default/geen `fonds_id`.
