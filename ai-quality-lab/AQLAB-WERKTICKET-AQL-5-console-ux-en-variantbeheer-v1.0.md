...
1. **Progressive disclosure:** het run-type stuurt welke velden verschijnen (baseline-kaart alleen bij regressie/subset; ad-hoc-vraag alleen bij ad_hoc; subset-selectie alleen bij subset). **Let op — stapnummering consistent:** de **modelkeuze (stap 2) blijft bij álle run-types zichtbaar**, óók bij ad-hoc (je moet altijd een model kiezen). Bij ad-hoc toont stap 2 alleen de modelkeuze ("Welk model gebruik je?") zonder baseline-vergelijking en zonder "gewijzigde as"; de stappen mogen nooit overslaan (nu springt de nummering van 1 naar 3 bij ad-hoc — dat is een bug).
...
7. **Empty-state-begeleiding** ("nog geen testset geseed → doe X").
8. **Ad-hoc consolideren + vereenvoudigen (BESLOTEN):** de run-type *"Ad-hoc testvraag"* wordt de **enige** ad-hoc-ingang en is **altijd niet-persistent** (`persist_mode = none`, direct/synchroon getest, resultaat meteen zichtbaar). Daarmee vervalt de aparte topknop *"Ad-hoc consistentietest"* — die functionaliteit (incl. consistentie-toggle + iteraties) zit al in het formulier. Bij `run_type = ad_hoc`: **persist-modus niet instelbaar** (vast op none, zichtbaar gelabeld); knop heet "Ad-hoc testen (niet bewaard)".
   - **Promotie naar testcase uit de UI (voor nu):** de topknop *"Ad-hoc vraag opslaan als testcase"* en de promoveren-flow gaan **uit de UI om de schermen eenvoudig te houden**. Nieuwe testcases komen via de seed/golden set, niet via in-app promotie. `lib/aqlab/promotie.ts` + de promoveren-route mogen geparkeerd/verborgen worden (niet noodzakelijk verwijderd); dit lost tegelijk de "promotie-zonder-persistente-run"-complicatie op. **Later** eventueel terug als het nodig blijkt.
   - **Topknoppenrij verdwijnt:** de losse actie-rij boven "Run samenstellen" vervalt volledig. *"Kwaliteit per feature"* (scherm 7) blijft bestaan maar wordt een **rustige nav-link** (header rechtsboven of bij het "Runs"-overzicht) — het is een monitoringbestemming, geen samenstel-actie; zo blijft de compose-form schoon.
   - **Audit:** log wél append-only in `aqlab_log` dát een ad-hoc test is uitgevoerd (wie/wanneer/model) — de *inhoud* wordt niet bewaard, de *gebeurtenis* wel (governance/herleidbaarheid), consistent met `persist_mode = none`.
...
- [ ] **Stapnummering consistent (geen oversprongen stap):** modelkeuze (stap 2) blijft bij ad-hoc zichtbaar als "Welk model gebruik je?" zonder baseline-vergelijking; nummering loopt altijd 1‑2‑3.
- [ ] **Ad-hoc = één ingang, altijd niet-persistent:** run-type ad_hoc forceert `persist_mode = none` (niet instelbaar); aparte "Ad-hoc consistentietest"-knop verwijderd; gebeurtenis append-only gelogd (inhoud niet).
- [ ] **Schermen vereenvoudigd:** de losse topknoppenrij vervalt volledig — "Ad-hoc consistentietest" verwijderd, "Ad-hoc vraag opslaan als testcase" + promoveren-flow uit de UI (geparkeerd); "Kwaliteit per feature" (scherm 7) verplaatst naar een rustige nav-link (header/naast runs-overzicht), niet boven de compose-form.
