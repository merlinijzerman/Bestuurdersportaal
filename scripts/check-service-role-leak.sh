...
echo "[2/3] lib/supabase-platform.ts begint met import \"server-only\"…"
if [ -f lib/supabase-platform.ts ]; then
  # De eerste import-statement (commentaarkop genegeerd) MOET server-only zijn.
  eerste_import="$(grep -m1 -E '^\s*import ' lib/supabase-platform.ts || true)"
  if ! echo "$eerste_import" | grep -q 'import "server-only";'; then
    melding "lib/supabase-platform.ts: eerste import is niet 'import \"server-only\";' (was: ${eerste_import:-<geen>})"
  fi
else
  melding "lib/supabase-platform.ts ontbreekt"
fi
...
if grep -rn --include='*.ts' --include='*.tsx' \
     -E "supabase-platform|platform-wrapper|platform-audit|platform-auth" \
