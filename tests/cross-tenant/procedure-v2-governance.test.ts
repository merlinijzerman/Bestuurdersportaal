...
test("bewijsbinding: het zetten of wijzigen van een binding wordt append-only gelogd", () => {
  assert.match(bewijsItemRoute, /event_type:\s*"bewijs_binding_gewijzigd"/);
  assert.match(bewijsItemRoute, /oud:\s*oudeSleutel/);
  assert.match(bewijsItemRoute, /nieuw:\s*nieuweSleutel/);
  assert.match(bewijsRoute, /requirement_sleutel:\s*bindingSleutel/);
});

test("bewijsbinding: de sleutel is uniek per stap — instantie-vereisten kunnen niet botsen", () => {
