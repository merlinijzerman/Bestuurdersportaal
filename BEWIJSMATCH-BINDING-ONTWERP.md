...
> **Status**: v1.1 — gereconstrueerd op `main` en gehard 22-08-2026
> **Aanleiding**: werkopdracht `WERKOPDRACHT-BEWIJSMATCH-BINDING.md`
> **Besluit**: `decisions/0183-expliciete-bewijs-vereiste-binding.md`
> **Migraties**: `supabase/migrations/2026_08_18_bewijs_requirement_binding.sql` + `2026_08_22_bewijs_requirement_binding_hardening.sql` (rollbacks onder `supabase/rollbacks/`)
...
| Gate | `supabase/migrations/2026_08_18_…sql` | Document-tak matcht op de binding; kolom + conservatieve backfill |
| DB-hardening | `supabase/migrations/2026_08_22_…sql` | Unieke indexes; configuratie- en bewijsvalidatietriggers; atomische audit; bewijs/readiness in snapshot |
| Binding | `core/lib/bewijs-binding.ts` (nieuw) | Server-side afleiden + verifiëren van de sleutel uit een triple |
| API | `app/api/procedures/[id]/bewijs/route.ts`, `…/[bewijsId]/route.ts` | `vereiste`-triple accepteren en valideren; logging gebeurt transactioneel in de DB |
...
Uitsluitingen wegen bewust niet mee bij het binden: binden aan een uitgesloten vereiste is onschadelijk (het telt nergens mee) en een uitsluiting kan worden ingetrokken, waarna de binding weer klopt. Een sleutel die meer dan één keer voorkomt over template- en instantie-arm wordt nergens willekeurig gekozen: resolver, DB-trigger, weergave en readiness-gate falen alle vier gesloten.
...
- Geen service-role en geen policywijziging. Drie nieuwe triggerfuncties zijn bewust `SECURITY DEFINER` met lege `search_path`; `public`, `anon` en `authenticated` hebben geen direct EXECUTE-recht. Zij leiden procedure en fonds uitsluitend af uit de gemuteerde requirement- of bewijsrij en de server-side relaties.
- De partiële index op `(stap_id, requirement_sleutel)` is uniek: één vereiste kan door maximaal één bewijsstuk worden geclaimd. `(procedure_id, volgorde)` is eveneens uniek voor stappen.
- Herkomst- en inhoudsvelden van een bewijsrij zijn immutable. Binding, documentkoppeling en documenttype zijn de enige mutaties en worden in dezelfde transactie naar `procedure_log` geschreven.
- Nieuwe beslismoment-snapshots bevatten stappen, bewijs inclusief bindingssleutels en het readiness-overzicht. Bestaande snapshots blijven ongewijzigd.
...
| Matchlogica | `core/lib/decision.sanity.ts` | 9 checks, incl. kernfixture en dubbele sleutel |
| Seed-hardening | `core/lib/procedure-requirements-seed.sanity.ts` | 6 checks (was 2) |
| Spiegeling TS↔SQL | `tests/cross-tenant/procedure-v2-governance.test.ts` | 11 bewijsbindingtests |
| Gate-gedrag (DB) | `supabase/checks/2026_08_18_bewijsbinding.sql` | structuur, gedrag, echte authenticated/RLS-write en tenantisolatie groen |
| Migratiereplay | wegwerp-Supabase/Postgres 17 | baseline + 16 post-baseline-migraties groen |
| Brede gates | typecheck, lint, sanity, cross-tenant appmatrix | groen; cross-tenant 251/251 |
...
|---|---|---|
| Bewijs opvoeren | `bewijs_toegevoegd` | `bewijs_id`, stap, titel, `document_id`, `stemming_id`, `requirement_sleutel` |
| Binding zetten/wijzigen/losmaken | `bewijs_binding_gewijzigd` | `bewijs_id`, stap, titel, `requirement_sleutel_oud`/`_nieuw` |
| Document/type koppelen | `bewijs_document_gekoppeld` | `bewijs_id`, stap, titel, oude/nieuwe `document_id` en `documenttype` |
| Bewijs verwijderen | `bewijs_verwijderd` | `bewijs_id`, stap, titel, `document_id`, `requirement_sleutel` (welke vereiste weer openvalt) |
| Stemverslag als bewijs | generieke `bewijs_toegevoegd`-trigger | `bewijs_id`, stap, titel, `stemming_id`, `requirement_sleutel: null` |
| Backfill | `bewijs_binding_backfill` | regels, aantallen, gelegde bindingen per rij, ongebonden ids |

Het stemverslag-pad (`app/api/stemmingen/[id]/sluiten/route.ts`) schreef eerder een `procedure_bewijs`-rij zónder logregel. De generieke DB-trigger vangt dit nu automatisch; het stuk draagt bewust géén binding — het systeem bepaalt niet welke bewijslast een stemverslag vervult.
...
3. **Vooraf bestaande sleutelbotsing template ↔ instantie.** Uniciteit over twee tabellen past niet in één gewone index. Nieuwe instantieconfiguratie wordt daarom door een SECURITY DEFINER-trigger tegen beide armen gecontroleerd; de route herhaalt die check voor een bruikbare foutmelding. De migratie normaliseert bestaande drift bewust niet stil. Zo'n al aanwezige botsing faalt gesloten in resolver, bewijsvalidatie, weergave en readiness en vraagt expliciet configuratieherstel voordat bewijs kan worden gekoppeld.
4. **`min_aantal` telt niet mee** voor document-vereisten — bestaand gedrag, ongewijzigd gelaten.
5. **Stemverslagen** worden als bewijsstuk ingevoegd zonder binding en vervullen dus niets tot iemand ze koppelt.
6. **Deployvolgorde.** Productie: eerst beide migraties, dan de code-deploy. Andersom selecteert `decision.ts` een kolom die nog niet bestaat.
