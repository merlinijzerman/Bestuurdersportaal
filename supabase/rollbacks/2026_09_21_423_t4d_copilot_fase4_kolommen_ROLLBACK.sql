...
-- ── Eindcontrole ────────────────────────────────────────────────────────────
do $$
declare v_n integer; v_rest text; v_rol text; v_tgnaam text; v_tgenabled text; v_tgtype integer; v_fnnaam text; v_fnschema text; v_muteerbaar boolean;
begin
  ...
  if v_tgnaam is null then
    raise exception 'FASE 4 mislukt: de trigger trg_copilot_operator_log_append_only ontbreekt op copilot_operator_log';
  end if;
  if v_fnschema <> 'microsoft_private' or v_fnnaam <> 'copilot_log_append_only' then
    raise exception 'FASE 4 mislukt: het auditslot hangt aan %.% in plaats van microsoft_private.copilot_log_append_only', v_fnschema, v_fnnaam;
  end if;
  if v_tgenabled not in ('O', 'A') then
    raise exception 'FASE 4 mislukt: het auditslot staat uit of vuurt alleen op een replica (tgenabled = %)', v_tgenabled;
  end if;
  if (v_tgtype & 16) = 0 or (v_tgtype & 8) = 0 then
    raise exception 'FASE 4 mislukt: het auditslot dekt niet zowel UPDATE als DELETE (tgtype = %)', v_tgtype;
  end if;
  if (v_tgtype & 1) = 0 or (v_tgtype & 2) = 0 then
    raise exception 'FASE 4 mislukt: het auditslot is niet BEFORE ... FOR EACH ROW (tgtype = %)', v_tgtype;
  end if;
  ...
  if exists (select 1 from microsoft_private.copilot_operator_log) then
    ...
    if v_muteerbaar then
      raise exception 'FASE 4 mislukt: een UPDATE op copilot_operator_log werd NIET geblokkeerd; het slot staat er wel maar werkt niet';
    end if;
  end if;
