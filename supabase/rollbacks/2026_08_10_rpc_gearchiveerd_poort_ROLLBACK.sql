-- ============================================================================
-- ROLLBACK 2026-08-10 — RPC-poort: gearchiveerd-clausule terugdraaien
-- ----------------------------------------------------------------------------
-- Herstelt zoek_chunks (2026_07_10) en zoek_chunks_hybride (2026_08_06) ZONDER
-- de `documentstatus is distinct from 'gearchiveerd'`-clausule. Verder identiek,
-- inclusief de ACL-hygiëne (revoke public,anon + grant authenticated,service_role).
-- ============================================================================
...
as $$
  ...
  fts as (
    select dc.id, row_number() over (order by ts_rank_cd(dc.zoek_vector, q.tsq) desc, dc.id) as r
    ...
    where d.actief = true
      ...
      and (p_modus is distinct from 'actueel' or (
      ...
      and (dc.bibliotheek is distinct from 'generiek' or (
  ...
  vec as (
    select dc.id, row_number() over (order by dc.embedding <=> p_embedding, dc.id) as r
    from public.document_chunks dc
    join public.documenten d on d.id = dc.document_id
    where d.actief = true
  ...
  select dc.id, dc.document_id, dc.tekst, dc.pagina, dc.paragraaf, dc.chunk_index,
         ...
         dc.bronorganisatie, dc.normgewicht, dc.extern_url, d.fonds_id, d.volgende_review
  from samen s
