-- P2 / PR-A (#167) — gebonden bewijs voor `assumption` op decision_assumptions.
...
alter table public.decision_assumptions
  add column if not exists requirement_sleutel text;
comment on column public.decision_assumptions.requirement_sleutel is
  'P2 (#167): binding aan een assumption-vereiste; formaat stap_volgorde|requirement_type|coalesce(documenttype,label); null = ongebonden.';

create index if not exists idx_assumption_req_sleutel
  on public.decision_assumptions(decision_id, requirement_sleutel)
  where requirement_sleutel is not null;

create or replace function public.fn_validate_assumption_binding()
...
begin
  ...
  perform public.fn_assert_gebonden_feit(v_fonds, v_proc, new.requirement_sleutel, 'assumption');
  return new;
end $$;
revoke all on function public.fn_validate_assumption_binding() from public, anon, authenticated;
...
begin
  ...
  perform public.fn_log_gebonden_feit_mutatie(v_proc, 'decision_assumptions', v_bron, v_oud, v_nieuw);
  return case when tg_op = 'DELETE' then old else new end;
end $$;
revoke all on function public.fn_audit_assumption_binding() from public, anon, authenticated;
grant execute on function public.fn_audit_assumption_binding() to service_role;
drop trigger if exists trg_assumption_audit_binding on public.decision_assumptions;
create trigger trg_assumption_audit_binding
  after insert or update or delete on public.decision_assumptions
  for each row execute function public.fn_audit_assumption_binding();
