-- ============================================================
--  Migratie 2026-08-14 — Fix: ambigue kolomreferentie 'blokkerend' in
--  fn_decision_readiness_check (regressie uit d7c, 2026-08-13).
--
--  De FOR-loop-SELECT selecteert de kolom `blokkerend` terwijl er een
--  gelijknamige PL/pgSQL-variabele `blokkerend` in scope is → PostgreSQL
--  gooit "column reference \"blokkerend\" is ambiguous" (42702). Daardoor
--  faalde fn_decision_readiness_overview en bouwde het Decision Object-dossier
--  niet op (page.tsx ving de fout op → dossier=null, lege Bewijsstukken).
--
--  Fix: `#variable_conflict use_column` bovenaan de functie zodat een ambigue
--  naam de KOLOM kiest (precies wat de SELECT nodig heeft; de variabele
--  `blokkerend` blijft elders eenduidig). Functie verder identiek aan d7c.
--
--  Idempotent (create or replace). Grant-hygiëne (Gate H) opnieuw toegepast,
--  want create-or-replace reset de ACL.
-- ============================================================
