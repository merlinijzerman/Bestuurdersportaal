...
--  ROLLBACK: supabase/rollbacks/2026_08_18_bewijs_requirement_binding_ROLLBACK.sql
-- ============================================================
...
-- Een inhoudelijk identieke sleutel in beide armen is juist GEEN eenduidige
-- kandidaat. UNION zou die twee definities stil tot één rij reduceren en de
-- backfill alsnog laten binden. Verwijder daarom elke (stap, sleutel) die meer
-- dan één definitie heeft voordat R1/R2 beginnen.
delete from _bind_kandidaten k
 using (
   select stap_id, sleutel
     from _bind_kandidaten
    group by stap_id, sleutel
   having count(*) <> 1
 ) ambigu
 where ambigu.stap_id = k.stap_id
   and ambigu.sleutel = k.sleutel;
...
--
--  Gebaseerd op 2026_08_14_readiness_uitsluiting.sql. De document-tak matcht
--  op de expliciete binding in plaats van documenttype-/titelheuristiek en
--  faalt gesloten bij een dubbele vereistesleutel of dubbele stapvolgorde.
