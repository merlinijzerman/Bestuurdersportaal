...
--    * copilot_lees_readiness        — leesfunctie voor microsoft_vault;
--    * copilot_zet_rollout /
...
create or replace function microsoft_private.copilot_lees_readiness(p_fonds uuid, p_gebruiker uuid)
returns table(
...
language sql security definer set search_path = microsoft_private, public, pg_temp as $$
  ...
  from verbindingen v
  where v.fonds_id = p_fonds and v.gebruiker_id = p_gebruiker
$$;
...
-- De applicatierol LEEST readiness maar bedient de rem niet.
