             ...
             where jsonb_typeof(e) <> 'object'
                ...
                or (e ? 'pii_soorten' and exists (
                  select 1 from jsonb_array_elements(e->'pii_soorten') p
                   where jsonb_typeof(p) is distinct from 'string' or length(p #>> '{}') > 64
                ))
                or (e ? 'versies' and (
         ...
         when jsonb_typeof(p_meta->'modelcontext_audit') = 'array'
          and not exists (
            select 1 from jsonb_array_elements(p_meta->'modelcontext_audit') e
             where jsonb_typeof(e) <> 'object'
                or (e - array['correlation_id','soort','pii','gerenderde_tekens','limiet','afgekapt','geneutraliseerd','fout']) <> '{}'::jsonb
                or jsonb_typeof(e->'correlation_id') is distinct from 'string'
                or length(e->>'correlation_id') not between 1 and 128
                or jsonb_typeof(e->'soort') is distinct from 'string'
                or length(e->>'soort') not between 1 and 64
...
do $$
declare
  v_meta jsonb := '{"evidence_audit":[{"correlation_id":"corr-368","soort":"semantische_unit","gevraagd":2,"toegelaten":1,"gerenderde_tekens":120,"limiet":500,"afgekapt":false}],"modelcontext_audit":[{"correlation_id":"corr-368","soort":"risicos","pii":"geen","gerenderde_tekens":80,"limiet":500,"afgekapt":false}]}'::jsonb;
begin
