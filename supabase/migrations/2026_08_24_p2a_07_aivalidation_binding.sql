-- P2 / PR-A (#167) — gebonden bewijs voor `ai_validation` op decision_ai_interactions.
...
comment on column public.decision_ai_interactions.requirement_sleutel is
  'P2 (#167): binding aan een ai_validation-vereiste; formaat stap_volgorde|requirement_type|coalesce(documenttype,label); null = ongebonden.';

create index if not exists idx_aivalidation_req_sleutel
  on public.decision_ai_interactions(decision_id, requirement_sleutel)
  where requirement_sleutel is not null;

create or replace function public.fn_validate_aivalidation_binding()
...
begin
  ...
  perform public.fn_assert_gebonden_feit(v_fonds, v_proc, new.requirement_sleutel, 'ai_validation');
  return new;
end $$;
revoke all on function public.fn_validate_aivalidation_binding() from public, anon, authenticated;
...
begin
  ...
  perform public.fn_log_gebonden_feit_mutatie(v_proc, 'decision_ai_interactions', v_bron, v_oud, v_nieuw);
  return case when tg_op = 'DELETE' then old else new end;
end $$;
revoke all on function public.fn_audit_aivalidation_binding() from public, anon, authenticated;
grant execute on function public.fn_audit_aivalidation_binding() to service_role;
drop trigger if exists trg_aivalidation_audit_binding on public.decision_ai_interactions;
create trigger trg_aivalidation_audit_binding
  after insert or update or delete on public.decision_ai_interactions
  for each row execute function public.fn_audit_aivalidation_binding();
