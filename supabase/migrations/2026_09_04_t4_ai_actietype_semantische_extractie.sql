-- #311 T4 — sluit het quotumgat voor fondsgebonden semantische extractie.
-- Forward-only aanvulling; de historische migratie uit augustus blijft intact.
begin;
...
as $$
  select s.bereik, s.ai_acties, s.via_gebruiker, s.via_systeem, s.lease_seconden
    from (values
      ...
      ('semantische_extractie',    'fonds',      1, false, true,   900),
...
comment on function public.fn_ai_actietype_spec(text) is
  'Bron van waarheid voor bereik, gewicht en lease per AI-actietype (besluit 0180, #311 T4). '
