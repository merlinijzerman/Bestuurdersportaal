-- P3 / PR-D (#168) — fn_stap_open_per_zwaarte: bind de open-berekening aan het
-- EXPLICIETE Decision Object. Besluit 0193. Reviewbevindingen HOOG #2/#3.
-- ---------------------------------------------------------------------------
-- De functie leidde het te beoordelen besluit af uit `is_primary_decision = true`
-- van de procedure. Die vlag is door `authenticated` updatebaar (RLS is `for all`
-- fonds-only) en er is geen uniciteit op één primary per procedure. Gevolg (bevestigd
-- in review): een aanroeper kon de primary-vlag naar een schone lokvogel verzetten,
-- of een SECUNDAIR besluit beslissen, waardoor de open-check naar de verkeerde
-- decision keek en de motiveringseis wegviel. Fix: p_decision_id wordt expliciet
-- meegegeven door fn_besluit_status_omslag; de PR-C afronden-functie roept nog met
-- één argument aan en valt via de default terug op de primary (gedragsbehoudend).
--
-- De 1-arg-variant wordt eerst gedropt: met een default-overload ernaast zou een
-- 1-arg-aanroep ambigu zijn ("function is not unique"). HAND-APPLIED. Rollback:
--   supabase/rollbacks/2026_08_28_p3d_04_open_per_decision_ROLLBACK.sql
