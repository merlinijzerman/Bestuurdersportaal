...
- **Deploy** verloopt via GitHub Desktop (commit → push). **Geen terminal-git commits**, met één nauw omschreven uitzondering: een AI-assistent mag `git worktree`, `git branch`, `git cherry-pick` en **lokale, ongepushte commits in een eigen worktree** gebruiken om een meerstapsopdracht in reviewbare stappen te knippen. **Push, merge, rebase op een gedeelde branch en het openen van een PR blijven uitsluitend handwerk in GitHub Desktop.** De regel beschermt de deployweg, niet het bestaan van lokale objecten; een reeks van acht ongecommitte wijzigingen in één werkboom is slechter reviewbaar én slechter terug te draaien dan acht lokale commits. Vastgelegd 3-9-2026 naar aanleiding van P1a, nadat de regel eerst was overtreden en gemeld. De weg is **preview eerst**: feature-branch → PR naar `preview` → merge → PR van `preview` naar `main`. `main` is de productiebranch en deployt naar Production; `preview` deployt naar de Preview-omgeving, die een **eigen Supabase** heeft (geverifieerd 22-08-2026). Branch protection op `main` staat aan en geldt óók voor de eigenaar: rechtstreeks pushen is geblokkeerd, en een PR naar `main` die niet van `preview` komt wordt rood gezet door de check **Previewpoort**. Bewuste uitzondering: label `hotfix-direct-naar-main`. Merge pas ná groene gates én een waargenomen preview-deploy — zie `security/RELEASEWEG-PREVIEW-EERST.md`.
