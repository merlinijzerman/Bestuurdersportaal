...
- **Deploy** verloopt via GitHub Desktop (commit → push → Vercel auto-deploy). **Geen terminal-git commits.** Zodra branch protection op `main` staat (V4): commit op een feature-branch → PR → merge ná groene gates, niet meer rechtstreeks op `main`.
...
**Blokkerend maken van de gates (V4).** De cross-tenant-, boundary- en security-baseline-workflows draaien op elke push/PR maar zijn **pas blokkerend als branch protection op `main` de bijbehorende jobs als _required status check_ selecteert** — dat is een repo-admin-handeling (Settings → Branches). Required checks pinnen op de **job-namen**, niet de workflow-namen: `Cross-tenant isolatie (§15 T1-T14)`, `Code-scheiding (T9 core/platform-grens)` en `Security baseline (Sprint 1)`. **Waarschuwing:** branch protection selecteert op die job-naam — hernoem een job nooit zonder de branch-protection-regel mee te verzetten, anders wordt de gate stil losgekoppeld (geen foutmelding). **Zodra branch protection aanstaat gaat álle werk via PR — ook, juist, AI-assisted werk;** de directe-push-flow naar `main` vervalt dan. Richt je de Claude Code GitHub Action in, geef dan géén `github_token: ${{ secrets.GITHUB_TOKEN }}` mee: met dat token start GitHub geen workflows op de agent-commits, dus draaien de gates niet op precies de PR's waar ze het hardst nodig zijn. Laat de parameter weg zodat de action als GitHub App authenticeert.
