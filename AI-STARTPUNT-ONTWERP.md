...
**Deel A — voorbeeldvragen bij de vrije vraag.** Een **vaste, generieke set** starters ([`core/lib/startvragen.ts`](./core/lib/startvragen.ts), `GENERIEKE_STARTVRAGEN`) — geen context-afleiding, geen koppeling, geen query. De chips verschijnen **pas nadat de gebruiker op "Een vrije vraag stellen" klikte** en alleen op de lege staat (patroon `STARTVRAGEN` in de vergader-AI). Een klik start de vraag als vrije vraag; het auditspoor krijgt een telemetrie-marker `retrieval_meta.startvraag_bron = "voorbeeldvraag"` (prefill vs. zelf getypt) — geen nieuwe tabel, geen nieuw `governance_events`-type.

> **Herziening 2026-07-29 (opdrachtgever).** De oorspronkelijke opzet (context-afgeleide vragen met twee generatoren `context`/`signaal`, spreiding over `vraagsoort`, herkomst-logging per generator) is vervangen door de vaste generieke set. Reden: een voorbeeldvraag die een specifiek stuk/agendapunt bij naam noemt zonder dat het gekoppeld is, "slaat nergens op" — en wie een documentvraag heeft, gebruikt taakkaart 2 ("Een document doorgronden"), die wél de scope zet. Daarmee vervallen de acceptatiecriteria 1/2/3 en de bijbehorende koppeling-/generator-machinerie (verwijderd). De eerdere scopebegrenzing rond `procedure_requirements`/procesfase is daarmee niet meer van toepassing.
