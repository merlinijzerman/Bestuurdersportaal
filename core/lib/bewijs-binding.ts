...
export async function resolveRequirementBinding(
  ...
  const templateTreffer = (templateRijen ?? []).find(
  ...
  if (templateTreffer) {
    return { ok: true, sleutel: doelSleutel, label: templateTreffer.label };
  }
  ...
  if (decisionIds.length > 0) {
    ...
    const instTreffer = (instRijen ?? []).find(
      (r: VereisteVerwijzing) =>
        requirementSleutel(
          r.stap_volgorde,
          r.requirement_type,
    ...
    if (instTreffer) {
      return { ok: true, sleutel: doelSleutel, label: instTreffer.label };
    }
  }
