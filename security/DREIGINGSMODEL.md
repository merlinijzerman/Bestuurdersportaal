...
| R-31 | Callback-replay, CSRF, `state`-hergebruik, open redirect | Hoog | Eenmalige versleutelde transactie (sha256(state) als sleutel, `gebruikt_op`), PKCE S256, nonce = sha256, `veiligVervolgpad`, vaste callback-URI per host (E4), hostcontrole in de blob | Gedekt in code (flowtests, E2E-replay); smoke N2/N3 |
...
| R-38 | Half-afgeronde koppeling (crash tussen `linkIdentity` en activeren) | Middel | Toestandsmodel; link + hook in één GoTrue-transactie (geen identiteit zonder reservering); idempotente herstelroute; `pending` verloopt na 10 min | Gedekt (flowtests herstel; T1 H6) |
...
| R-40 | Hookfout of -uitschakeling raakt wachtwoordlogin of schakelt de controle uit | Kritiek | Hook keert voor niet-`oauth` terug vóór databaseraadpleging; `oauth` fail-closed; guard L3 als tweede signaal; wachtwoordpad byte-identiek (W1, LK-INVARIANTS) | Deels — smoke P4/P8 direct na inschakelen |
