...
| R-43 | Break-glass wordt een permanente achterdeur of wordt zelf toegekend | Hoog | `check (user_id <> uitgegeven_door)`, verplichte einddatum (60 s – 7 dagen), vaste redencategorie (geen vrije tekst), en pas werkzaam mét een geverifieerde MFA-factor; elke verlening, weigering en intrekking staat inhoudsvrij in `login_private.audit_log` | Gedekt (M5, M7, M12); operationeel signaal op herhaald gebruik volgt in PR-B |
