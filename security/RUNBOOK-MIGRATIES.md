...
**Ontwerp:** `02 Architectuur/ONTWERPNOTITIE-MIGRATIEPROCES-v2.1.md` is leidend voor het waarom en voor de inrichting.
**Versie:** 1.2 · 19 augustus 2026 — §C bijgewerkt op v2.1: fase 1 (de verplaatsing) is uitgevoerd, de cijfers zijn hermeten tegen `origin/main`, en drie beweringen uit v1.1 bleken onjuist. Zie de voetnoot onder §C.
...
Uit `ONTWERPNOTITIE-MIGRATIEPROCES-v2.1.md` §12. De volgorde is niet vrij.
...
- [x] **Beantwoord 19-08-2026:** `2026_08_15_t14b_production_drift_repair.sql` is **niet toegepast**; de drift staat er nog. Het uitvoerplan `security/PRODUCTIE-UITVOERPLAN-AUDITKETEN-T14B-2026-08-15.md` staat op NO-GO met een leeg go/no-go-record. Live gemeten met `supabase/checks/2026_08_19_t14b_driftmeting.sql`: de volledige rij-capture ontbreekt, de no-op-guard is aanwezig — en juist die combinatie zorgt dat wijzigingen in twaalf inhoudskolommen (o.a. `kpi.toelichting`, `reeks.kleur`, `delta`) **helemaal geen** logregel opleveren. Twee onderdelen (anti-spoofing, RPC-typevalidatie) nog te meten met diezelfde check.
- [x] **Productiemajor vastgesteld 19-08-2026** (dashboarduitlezing, zie hieronder bij fase 4).

**Fase 1a — de verplaatsing (uitgevoerd 19-08-2026, branch `chore/migratie-mapindeling`)**
- [x] 125 `*_ROLLBACK.sql` naar `supabase/rollbacks/`. **Kritiek gebleven:** de CLI past élk geldig SQL-bestand in `supabase/migrations/` toe. Van de 286 bestanden daar waren er 137 géén forward-migratie.
- [x] 2 `*_seed_preview.sql` naar `supabase/seeds/preview/`; 10 overige seeds naar `supabase/seeds/schema/`.
- [x] `scripts/testdb-apply-migrations.sh` op de nieuwe indeling; **beide** naamfilters vervallen. Bewezen dat de runner exact dezelfde tien post-baseline-migraties selecteert als daarvoor.
- [x] CI-gate `scripts/check-migratie-mapindeling.sh`, als gepinde job *"Mapindeling supabase/ (migraties vs rollbacks/seeds)"* in `boundaries.yml`. Positief én negatief getest.
- [ ] **Nog te doen:** die job aanzetten als *required status check* op `main` — één repo-adminhandeling.
- [x] De tien schema-seeds per bestand geclassificeerd: zie `supabase/seeds/README.md`. Twee horen in de baseline, één is vervangen, één is omgevingsspecifiek, zes zijn demonstratiedata met onderlinge volgorde-afhankelijkheid.

**Fase 1b — de hernummering (bewust uitgesteld)**
- [ ] 149 forward-migraties hernoemd naar `^[0-9]{14}_`, met `HERNUMMERING-2026-08.md` als mapping. Raakt 111 documenten en 128 bestandsnamen. Pas nodig bij fase 3, want dán gaat de CLI-ledger de namen lezen. Neem de negen lettersuffix-bestanden expliciet op, inclusief de volgordekeuze bij de twee `2026_06_20e`-bestanden.
...
- [x] **Versiegelijkheid op major — vastgesteld 19-08-2026.** Productie draait Postgres `17.6.1.111`, de test-DB is gepind op `major_version = 17`. Het scenario "productie mogelijk nog op 14" bestond niet; `supabase/config.toml` is gecorrigeerd.
- [ ] **Minorverschil beleggen.** Preview draait `17.6.1.155` en PostgREST `14.15`; productie `17.6.1.111` en PostgREST `14.5`. Preview loopt dus op twee assen vóór op de omgeving die hij hoort te voorspellen. Trek gelijk **vóór** de poort in gebruik gaat, niet ertussendoor.
- [ ] **Geslaagde restore-oefening of werkende PITR-procedure, met gemeten RTO.** Een post-migratiegate kan een gecommitteerde wijziging niet terugdraaien. De oefening van 15-08 kwam tot een groene applicatierestore (118 `public`-tabellen) maar liep vast op de volledige datarestore: de losse Postgres-image had niet dezelfde managed `auth`-schemaversie als het bronproject. Herhaal naar een **leeg Supabase-doelproject**. "Geslaagd" is een meting, geen commit.
- [ ] **Proportionaliteit vastleggen.** Welke klasse wijziging vraagt een bewezen volledige restore, en welke volstaat met een werkende rollback plus groene gates? Zonder dat onderscheid staat elke migratie — ook een additieve lege tabel — achter dezelfde geblokkeerde oefening in de rij, en die rij is zelf een risico.

---

> **Voetnoot bij §C (19-08-2026).** Drie beweringen uit v1.1 bleken bij narekening onjuist en zijn hierboven gecorrigeerd:
>
> 1. *"De huidige runner past de tien seeds toe; blind weghalen breekt de test-DB."* — Onjuist. Alle twaalf seeds sorteren vóór het baseline-cutoff, dus de runner slaat ze over; hun inhoud zit al in de gesquashte Preview-baseline. Verplaatsen was risicoloos.
> 2. De aantallen (282 / 135 / 123 / 147) kwamen uit een meetbasis die niet op `origin/main` lag. Hermeten: 286 / 137 / 125 / 149.
> 3. *"Versiegelijkheid"* werd als open blokkade opgevoerd op grond van een vermoeden in `config.toml` dat productie mogelijk op major 14 draaide. Gemeten: major 17, gelijk aan de test-DB.
>
> De gemene deler is dat alle drie uit documentatie kwamen in plaats van uit een meting. Dat is de reden dat §C nu per regel vermeldt wanneer iets gemeten is.
