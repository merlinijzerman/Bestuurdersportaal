...
- **Contractvragen:** V1–V8, V10 en V12 gesloten conform het voorstel; V9 = dedicated limiet `microsoft_login_start` per ip+host, 20 per 10 minuten, **atomisch geteld in de private gateway** (reviewcorrectie PR #339, zie §8); V11 = één neutrale melding voor `error=auth_callback` én `fout=…` zonder technische details.
- **Reviewronde PR #339 (2026-09-07) verwerkt:** (1) browsergrens — publieke teksten/codes in de importloze `microsoft-login-meldingen-core.ts`; clientcomponenten importeren uitsluitend die module; regressietest `tests/cross-tenant/microsoft-login-t2-contract.test.ts` (transitieve waarde-imports, proven-red op de oude keten); `npm run build` groen. (2) V9 — `login_private.tel_startpoging` (veertiende gatewayfunctie, migratie `2026_09_07_microsoft_login_startlimiet.sql` + rollback + check-suite in `cross-tenant-ci.sh`), sleutel = HMAC-SHA256(ip|host) onder de loginsleutel, telling mislukt = weigeren; de in-geheugen teller is verwijderd. (3) B7 uitgevoerd: vier paginascenario's opgenomen tegen de wegwerpstack, 378/378 driemaal stabiel. (4) Ontkoppelen deterministisch: de status meldt `sessieViaMicrosoft`; een oauth-sessie wordt server-side beëindigd (`uitgelogd: true`) en de kaart stuurt naar `/login`; een wachtwoordsessie blijft (`uitgelogd: false`) en de kaart zegt dat vooraf.
- **Reviewronde 2 PR #339 (2026-09-07) verwerkt — hostgrens:** de ruwe `Host`-header werd na een genormaliseerde fondscontrole voor redirects gebruikt (`pgb.example:443@evil.example` → fondscontrole `pgb.example`, als URL een redirect naar `evil.example`), en de callback viel bij een onbekende host terug op `requestOrigin`. Nu: één strikte canonicalisering `canoniekeFondsHost()` (flow-core; alleen DNS-labelvorm in kleine letters, geen userinfo/pad/`?`/`#`/witruimte/haakjes; productie zonder poort; een poort alleen lokaal op `.localhost`/`localhost`/`127.0.0.1`) — dezelfde waarde voor fondscontrole, limietsleutel, callback-URI en redirect-origin; `origineVoorHost`/`callbackUrlVoorHost` weigeren een niet-canonieke host hard; ongeldige of onbekende host → neutrale 404 op start én callback, nooit een redirect uit `req.url`. Negatieve tests in `microsoft-login-flow-core.sanity.ts` (22 gevallen) en een contracttest die elk gebruik van de Host-header aan `canoniekeFondsHost` bindt.
...
| `telStartpoging({sleutel, limiet, vensterSeconden})` (T2, V9) | `sleutel` = 64 hex (HMAC) | `{toegestaan, resterend, resetOp}` | `login_private.tel_startpoging`, atomisch per vast venster; de veertiende functie voor `login_gateway` (F1B-check telt 14) |
...
| **B7** ✅ | Vier W1-paginascenario's (`w335.auth-callback.get.anon.zonder-code`, `.ongeldige-code`, `w335.login.get.anon`, `w335.login.get.bestuurder.307`) opgenomen tegen de wegwerpstack en driemaal geverifieerd: **378/378 ok** (0 fout, 4 SSE-stub-scenario's zichtbaar overgeslagen). De 374 bestaande snapshots bleven byte-identiek mét guard L3 in de wrapper en L4 in de callback. De vier nieuwe API-routes blijven uitgestelde opnames (W1-omgeving zonder Microsoft-config) | `karakterisering.yml` | in PR-B |
...
Besluit opdrachtgever (2026-09-07): V1–V8, V10 en V12 conform het voorstel; **V9** = dedicated rate limit `microsoft_login_start`, per IP + host, 20 pogingen per 10 minuten; **V11** = één neutrale foutmelding voor zowel `error=auth_callback` als `fout=…`, zonder technische details. Uitvoering: V9 → atomische teller `login_private.tel_startpoging(p_sleutel, p_limiet, p_venster_seconden)` (migratie `2026_09_07_microsoft_login_startlimiet.sql`; INSERT … ON CONFLICT DO UPDATE … RETURNING op een vast venster; tabel `start_pogingen` zonder rolrechten; execute alleen `login_gateway` — de veertiende gatewayfunctie), aangeroepen via `gateway.telStartpoging` op `/auth/microsoft-login/start` met sleutel HMAC-SHA256(ip|host) onder de loginsleutel (`microsoft-login-ratelimit-core.ts`); telling mislukt = weigeren. De eerste versie (in-geheugen per instantie) is op review verworpen en verwijderd. Plus `LIMIETEN.microsoft_login_start` per gebruiker op de koppel-start; V11 → `LOGIN_MICROSOFT_MELDING` in `microsoft-login-error-core.ts`, gerenderd door `app/login/page.tsx` voor beide parameters. V10 → RS256 via `node:crypto` (`microsoft-login-oidc-core.ts`). V8 → `LOGIN_GATEWAY_*` in `karakterisering.yml` en `e2e-security.yml`. De tabel hieronder is de oorspronkelijke vraaglijst.
